...
PROFILES = [
    Profile(
        InputTemplate('uvpipeline-txt-input', PlainTextFormat,"Raw txt file",
        ...
        #------------------------------------------------------------------------------------------------------------------------
        OutputTemplate('uvpipeline-naf-output',NafXMLFormat,'Naf NLP file',
            SetMetaField('encoding','utf-8'), #note that encoding is required if you work with PlainTextFormat
            extension='.naf', #set an extension or set a filename:
            # filename='output.naf',
            unique=True
        ),
    )
...
#
COMMAND = WEBSERVICEDIR + "/vupipeline_wrapper.py $DATAFILE $STATUSFILE $OUTPUTDIRECTORY"

#Or for the shell variant:
#COMMAND = WEBSERVICEDIR + "/vupipeline_wrapper.sh $STATUSFILE $INPUTDIRECTORY $OUTPUTDIRECTORY $PARAMETERS"
