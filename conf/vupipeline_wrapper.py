...
inputfile = clamdata.inputfile('uvpipeline-txt-input')
inputfilepath = str(inputfile)

outputfilepath = ''
for outputfile, outputtemplate_id in clamdata.program.getoutputfiles():
  if outputtemplate_id == 'uvpipeline-naf-output':
        outputfilepath = str(outputfile) #example showing how to obtain the path to the file
...
os.system('/vurm/scripts/run-pipeline.sh < ' + inputfilepath + ' > ' + outputfilepath);
...
clam.common.status.write(statusfile, "Pipeline done", 100) # status update
